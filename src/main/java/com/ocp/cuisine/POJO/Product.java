...
@NamedQuery(name = "Product.getProductByCategoryId", query = "SELECT new com.ocp.cuisine.wrapper.ProductWrapper(product.id, product.name, product.category, product.description, product.price, product.status) FROM Product product WHERE product.category.id =:id AND product.status ='true'")
...
@NamedQuery(name = "Product.searchProducts", query = "SELECT new com.ocp.cuisine.wrapper.ProductWrapper(product.id, product.name, product.description, product.price) FROM Product product WHERE " +
                                                        "product.name LIKE CONCAT('%',:query, '%')" +
                                                        "OR product.description LIKE CONCAT('%', :query, '%')")
